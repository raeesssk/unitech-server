...
router.post('/edit/:quotationId', oauth.authorise(), (req, res, next) => {
  ...
  const addMaterial=req.body.addMaterial;
  ...
  pool.connect(function(err, client, done){
    ...
    client.query(singleInsert, params, function (error, result) {
        ...
        removeMaterial.forEach(function(product, index) {
            client.query("delete from final_quotation_product_master where fqpm_id = $1",
                  [product.fqpm_id]);
        });

        purchaseMultipleData.forEach(function(product, index) {
            client.query("update final_quotation_product_master set fqpm_item=$1, fqpm_quantity=$2, where fqpm_id = $3",
                  [product.fqpm_item,product.fqpm_quantity,product.fqpm_id]);
        });

        addMaterial.forEach(function(product, index) {
