...
router.post('/add', oauth.authorise(), (req, res, next) => {
  ...
  pool.connect(function(err, client, done){
    ...
    var singleInsert = 'INSERT INTO quatation_master(qm_design_no, qm_quotation_no, qm_cm_id, qm_date, qm_ref_no, qm_status) values($1,$2,$3,$4,$5,0) RETURNING *',
        params = [quotation.qm_design_no.dm_design_no,quotation.qm_quotation_no,quotation.qm_cm_id.cm_id,quotation.qm_date,quotation.qm_ref_no];
...
router.get('/product/:quotationId', oauth.authorise(), (req, res, next) => {
  ...
  pool.connect(function(err, client, done){
    ...
    // SQL Query > Select Data
    const query = client.query('SELECT * FROM quotation_product_master qpm left outer join quatation_master qm on qtm.qtm_qm_id=qm.qm_id where qm_id=$1',[id]);
    query.on('row', (row) => {
      results.push(row);
    });
    query.on('end', () => {
...
router.get('/view/:quotationId', oauth.authorise(), (req, res, next) => {
  ...
  pool.connect(function(err, client, done){
    ...
    const query = client.query("SELECT * FROM quotation_product_master qtm inner join quatation_master qm on qtm.qtm_qm_id=qm.qm_id where qm_id=$1",[id]);
    query.on('row', (row) => {
      results.push(row);
